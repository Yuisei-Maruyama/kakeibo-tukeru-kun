...
**処理内容:**
- GoogleカレンダーからJST当日の予定（colorId:7/8/10 または タイトルに「予定」を含む）を取得
- ユーザーごとに予定をまとめ、開始・終了時刻または終日表示つきでLINEグループに通知
- 予定がない場合は通知しない
...
`year` と `month` は両方指定する。未指定の場合はJST現在月・先月・先々月を対象にする。
...
| カテゴリー | colorId | 色 | 用途 |
|------------|---------|------|------|
| 外食費用 | 4 | フラミンゴ | レストラン・飲食店 |
| 買い物費用 | 5 | バナナ | スーパー・コンビニ |
| 旅行費用 | 1 | ラベンダー | 旅行関連の支出 |
| 家賃費用 | 10 | バジル | 月末家賃 |
| 予定 | 8 / 7 | グラファイト / ピーコック | ユーザー別スケジュール |
...
|----------|------|-----|
| `@追加` | 対話形式で支出を登録 | @追加 |
| `@追加 {支払い者名} {カテゴリー} {金額} [{日付}] [{内容}] [{メモ}]` | 一括入力で支出を追加。内容は従来の店舗名/支払い内容として扱い、メモは任意で保存 | @追加 田中 外食費用 1280<br>@追加 @自分 外食費用 1280<br>@追加 田中 外食費用 1280 12/1 ランチ<br>@追加 田中 外食費用 1280 2024/12/1 ランチ 打ち合わせ後 |
| `@削除` | 対話形式で支出を削除 | @削除 |
| `@削除 {カテゴリー} {支払い者名} {金額} [{日付}]` | 指定条件の支出を削除（日付は省略可） | @削除 外食費用 @自分 1280<br>@削除 外食費用 @自分 1280 12/3<br>@削除 買い物費用 田中 2500 2024/5/22 |
