...
| 受領完了ノート | 月間の貯金・精算を確認（未確認/確認済みタブ・行単位トグルで確認日を JST 自動記録、両者確認で「確認完了」）。確認を付けるときはカテゴリー別メッセージの確認モーダルで確認を取ってから記録する（解除は即時）。買い物/旅行の精算は @集計 と同じく支払額が少ない側が差額の半分を多い側に返金する 1 行を表示。手動の明細追加は「その他」カテゴリー専用（対象月・タイトル・金額を指定、過去月＝開始月〜当月にも追加可）。外食貯金・買い物/旅行費用精算は自動集計行から確認する。その他は家賃設定の金額で「家賃代」の自動行を毎月表示（確認・編集・削除は他の自動行と同様）。既存明細の編集モーダルではカテゴリーは変更不可（表示のみ）で、その他行はタイトル・設定額／それ以外は対象ユーザー・設定額を編集する | Firestore `receiptNotes.confirmations` / `POST・PATCH・DELETE /api/receipt-notes` / `PUT /api/receipt-notes/{id}/confirmation` |
| キャンセル | 対話キャンセル | `@キャンセル` |
