...
| 受領完了ノート | 月間の貯金・精算をユーザー別に確認（未確認/確認済みタブ・行単位トグルで確認日を JST 自動記録、両者確認で「確認完了」） | Firestore `receiptNotes.confirmations` / `POST・PATCH・DELETE /api/receipt-notes` / `PUT /api/receipt-notes/{id}/confirmation` |
| キャンセル | 対話キャンセル | `@キャンセル` |
